...
public class ConsoleSession {
			...
			public void run() {
				...
				if (!queryPoolActive) {
					...
					for (int i = 0; i < queryPool.size(); i++) {
						...
						tmpIntent.setAction(StaticClass.ARMITAGE_CONSOLE_WRITE);
	...
	private void read() {
		Intent tmpIntent = new Intent();
		tmpIntent.setAction(StaticClass.ARMITAGE_CONSOLE_READ);
		tmpIntent.putExtra("id", id);
		tmpIntent.putExtra("msfId", msfId);
		context.sendBroadcast(tmpIntent);
	}
	...
	public void destroy() {
		...
		tmpIntent.putExtra("msfId", msfId);
		tmpIntent.putExtra("id", id);
		tmpIntent.setAction(StaticClass.ARMITAGE_CONSOLE_DESTROY);
		context.sendBroadcast(tmpIntent);
	}
