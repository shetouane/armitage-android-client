...
import com.shetouane.armitage.structures.ModulesAdapter;
			...
			for (ModuleItem temp : MainService.modulesMap.ExploitItems) {
				...
				if (res != null && MainService.databaseHandler != null) {
					try {
						...
						MainService.databaseHandler.updateModuleOptions(temp,
				 ...
				 * String port =
				 * res.get("RPORT").asMapValue().get("default").asRawValue
				 * ().getString();
				 * 
				 * if (!portsMap.containsKey(port)) portsMap.put(port, new
				 * ArrayList<String>());
				 * 
				 * portsMap.get(port).add(temp.getPath());
				 * 
				 * if (port.equals("80")) { if (!portsMap.containsKey("443"))
				 * portsMap.put("443", new ArrayList<String>());
				 * 
				 * portsMap.get("443").add(temp.getPath()); } else if
				 * (port.equals("443")) { if (!portsMap.containsKey("80"))
				 * portsMap.put("80", new ArrayList<String>());
				 * 
				 * portsMap.get("80").add(temp.getPath()); } else if
				 * (port.equals("445")) { if (!portsMap.containsKey("139"))
				 * portsMap.put("139", new ArrayList<String>());
				 * 
				 * portsMap.get("139").add(temp.getPath()); } else if
				 * (port.equals("139")) { if (!portsMap.containsKey("445"))
