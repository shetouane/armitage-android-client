...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	public static final String DATABASE_NAME = "armitage.db";
	private static final int DATABASE_VERSION = 1;

	// Database creation sql statement
	private static final String[] DATABASE_CREATE = {
			"create table " + TABLE_EXPLOITS + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",

			"create table " + TABLE_PAYLOADS + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",

			"create table " + TABLE_POSTS + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",

			"create table " + TABLE_ENCODERS + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",

			"create table " + TABLE_NOPS + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",

			"create table " + TABLE_AUXILIARY + "(" + COLUMN_MODULE_ID
					+ " integer primary key autoincrement, "
					+ COLUMN_MODULE_PATH + " text not null, "
					+ COLUMN_MODULE_OPTIONS + " blob, " + COLUMN_MODULE_INFO
					+ " blob);",
	...
	public static final String TABLES[] = { TABLE_EXPLOITS, TABLE_PAYLOADS,
			TABLE_POSTS, TABLE_ENCODERS, TABLE_NOPS, TABLE_AUXILIARY };
	...
	public synchronized void addModule(String path, Map<String, Value> info,
			Map<String, Value> options, String type) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			db.execSQL("PRAGMA synchronous=OFF");
			db.beginTransaction();
	...
	public synchronized ModuleItem getModule(int id, String type) {
		...
		Cursor cursor = db.query(TABLES[getIdByString(type)], new String[] {
				COLUMN_MODULE_ID, COLUMN_MODULE_PATH },
				COLUMN_MODULE_ID + "=?", new String[] { String.valueOf(id) },
				null, null, null, null);
		if (cursor != null)
			cursor.moveToFirst();
	...
	public synchronized List<String> getAllModulesString(String type) {
		...
		String selectQuery = "SELECT " + COLUMN_MODULE_PATH + " FROM "
	...
	public synchronized int getModulesCount(String type) {
		String countQuery = "SELECT COUNT(" + COLUMN_MODULE_ID + ") FROM "
				+ TABLES[getIdByString(type)];
		SQLiteDatabase db = this.getReadableDatabase();
	...
	public synchronized int updateModule(ModuleItem item, String type) {
		...
		ContentValues values = new ContentValues();
		values.put(COLUMN_MODULE_PATH, item.getPath());
		int ret = db.update(TABLES[getIdByString(type)], values,
				COLUMN_MODULE_ID + " = ?",
	...
	public synchronized void updateModuleOptions(ModuleItem item, String type,
			byte[] options) {
		SQLiteDatabase db = this.getWritableDatabase();
		...
		db.update(TABLES[getIdByString(type)], params, COLUMN_MODULE_ID + " = "
				+ item.getID(), null);
		db.close();
	}
