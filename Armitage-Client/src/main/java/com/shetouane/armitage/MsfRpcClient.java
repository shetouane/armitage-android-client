...
public class MsfRpcClient {
	...
	MsfRpcClient(Context c, Map<String, Object> opts) {
		...
		host = opts.containsKey("host") ? opts.get("host").toString()
				: "127.0.0.1";
		...
		client = (ssl ? sslClient(new DefaultHttpClient(cm, params))
	...
	public boolean login(String username, String password) {
		HttpPost httpPost = newHttpPost(encode(new String[] { "auth.login",
				username, password }));
		Map<String, Value> res = getResponse(httpPost);

		if (res != null) {
			if (res.containsKey("result")
					&& res.get("result").asRawValue().getString()
							.equals("success")) {
				token = res.get("token").asRawValue().getString();

				httpPost = newHttpPost(encode(new String[] {
			...
			tmpIntent.setAction(StaticClass.ARMITAGE_AUTHENTICATION_FAILED);
	...
	private Map<String, Value> getResponse(HttpPost p) {
		try {
			...
			Intent tmpIntent = new Intent();
			tmpIntent.setAction(StaticClass.ARMITAGE_CONNECTION_TIMEOUT);
			context.sendBroadcast(tmpIntent);
			...
			Intent tmpIntent = new Intent();
			tmpIntent.setAction(StaticClass.ARMITAGE_CONNECTION_FAILED);
			context.sendBroadcast(tmpIntent);
	...
	public String[] getModules(String type) {
		...
		if (type.contains(StaticClass.ARMITAGE_LOAD_EXPLOITS))
			moduleTag += "exploits";
		else if (type.contains(StaticClass.ARMITAGE_LOAD_PAYLOADS))
			moduleTag += "payloads";
		else if (type.contains(StaticClass.ARMITAGE_LOAD_AUXILIARY))
			moduleTag += "auxiliary";
		else if (type.contains(StaticClass.ARMITAGE_LOAD_POSTS))
			moduleTag += "post";
		else if (type.contains(StaticClass.ARMITAGE_LOAD_ENCODERS))
			moduleTag += "encoders";
		else if (type.contains(StaticClass.ARMITAGE_LOAD_NOPS))
			moduleTag += "nops";
		else
			return null;
		...
		if (res != null && res.containsKey("modules")) {
	...
	private Template<Map<Integer, Value>> decodeAsIntMap = tMap(TInteger,
			TValue);
	...
	private Map<String, Value> decode(InputStream in) throws IOException {
		...
		try {
			...
			Map<Integer, Value> before = new Converter(tmpResult)
					.read(decodeAsIntMap);
			Map<String, Value> after = new HashMap<String, Value>();

			Integer[] keys = before.keySet()
					.toArray(new Integer[before.size()]);
